...
window.addEventListener('scroll', () => {
  ...
  if(window.innerHeight > remainedBeanPos) {
    const beanImg = containerBean.querySelector('.container__bean__wrapper__img');
    const beanContent = containerBean.querySelector('.container__bean__content');
  ...
  if(window.innerHeight > remainedReservePos) {
    const reserveRight = containerReserve.querySelector('.container__reserve__right');
